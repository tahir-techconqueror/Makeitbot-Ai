...
/**
 * Brand Hero with Video Background
 *
 * Enhanced hero component with video background support.
 */
...
import { MapPin, ArrowRight, CheckCircle, Star, Leaf, Truck, ShoppingCart, Volume2, VolumeX } from 'lucide-react';
import { cn } from '@/lib/utils';

interface BrandHeroVideoProps {
  brandName: string;
  brandLogo?: string;
  tagline?: string;
  description?: string;
...
export function BrandHeroVideo({
  ...
  const [isMuted, setIsMuted] = useState(videoBackground?.muted !== false);
  ...
  useEffect(() => {
    if (videoRef.current && videoBackground?.autoplay) {
      videoRef.current.play().catch(console.error);
    }
  }, [videoBackground?.autoplay]);
      ...
      {videoBackground ? (
        <>
          ...
          <video
            ...
            poster={videoBackground.posterImage}
            muted={isMuted}
            loop={videoBackground.loop !== false}
            autoPlay={videoBackground.autoplay !== false}
            playsInline
            onLoadedData={() => setIsVideoLoaded(true)}
          >
            <source src={videoBackground.url} type="video/mp4" />
          </video>
