...
async function initializeState() {
  ...
  console.log('[Markitbot] Background initialized', {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  handleMessage(message, sender)
    .then(sendResponse)
    .catch((error) => {
      console.error('[Markitbot] Message handling error:', error);
...
async function handleMessage(message, sender) {
  ...
  switch (type) {
    ...
    default:
      console.warn('[Markitbot] Unknown message type:', type);
...
async function handleRecordAction(payload, sender) {
  ...
  if (state.recordingId) {
    try {
      ...
      console.error('[Markitbot] Failed to sync action:', error);
    }
  }
...
function createContextMenu() {
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({
      id: 'markitbot-record-click',
      title: 'Record Click on Element',
      contexts: ['all'],
    });

    chrome.contextMenus.create({
      id: 'markitbot-screenshot',
    ...
    chrome.contextMenus.create({
      id: 'markitbot-start-recording',
      title: 'Start Recording',
      contexts: ['page'],
    });

    chrome.contextMenus.create({
      id: 'markitbot-stop-recording',
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  switch (info.menuItemId) {
    ...
    case 'markitbot-start-recording':
      if (state.recordingState === RECORDING_STATES.IDLE) {
...
chrome.runtime.onInstalled.addListener(async (details) => {
  console.log('[Markitbot] Extension installed/updated:', details.reason);
...
chrome.runtime.onStartup.addListener(async () => {
  console.log('[Markitbot] Extension started');
  await initializeState();
  createContextMenu();
});
